...
def build_prompt(comments):
    ...
    prompt = "Provide a concise overview of this professor based on student comments. Use definitive language, be direct, and limit to 3 sentences maximum. Focus on overall teaching quality, not individual student experiences:\n\n"
...
def update_professor(professor, summary):
    """
    Reorder the keys in the professor dictionary to insert the summary after numRatings.
    If 'numRatings' does not exist, the summary is added at the end.
    """
    new_prof = {}
...
def process_json_file(input_path):
    ...
    with concurrent.futures.ThreadPoolExecutor() as executor:
        future_to_index = {executor.submit(get_llm_summary, comments): index for index, comments in tasks}
